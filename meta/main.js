...
function processCommits(data) {
  return d3
    ...
    .map(([commit, lines]) => {
      ...
      let ret = {
        id: commit,
        url: 'https://github.com/Siddhu-S/portfolio/commit/' + commit,
        author,
        date,
        time,
        timezone,
...
function renderCommitInfo(data, commits) {
  ...
  dl.append('dd').text(longestFile ? `${longestFile[0]} (${longestFile[1]} lines)` : 'N/A');
}
...
function isCommitSelected(selection, commit) {
  ...
  // selection = [[x0, y0], [x1, y1]] in screen coords
...
dots
  ...
  .on('mouseleave', () => {
    document.getElementById('commit-tooltip').hidden = true;  // ← hide
  });
